...
local Window = Library:CreateWindow({
    Title = 'Ion Hub | Made By 0Windycaketi | Map 1',
    Center = true,
    AutoShow = true,
    TabPadding = 8,
    MenuFadeTime = 0.4
...
local TabboxFarms = Tabs.Main:AddLeftTabbox()

local FarmSetting = TabboxFarms:AddTab("[ Setting ]")

getgenv().Method = "Sword"
FarmSetting:AddDropdown('KillAura', {
    Values = { 'Sword', 'Combat', 'Scythe', 'Claw', 'Fans' },
    Default = 1,
    Multi = false,
    Text = 'Kill Aura Method',
    Tooltip = 'Select Kill Aura Method',
...
FarmSetting:AddDropdown('FarmMethod', {
...
FarmSetting:AddSlider('TweenSpeed', {
    Text = 'Tween Speed',
    Default = 100,
    Min = 10,
    Max = 500,
...
getgenv().Distance = 5
FarmSetting:AddSlider('FarmDistance', {
    Text = 'Farm Distance',
    Default = 5,
    Min = 1,
    Max = 10,
...
FarmSetting:AddDivider()

FarmSetting:AddToggle('RemoveDMG', {
    Text = 'Remove Damage Text',
    Default = false,
    Tooltip = 'Remove Damage Text',
    Callback = function()
        RemoveDMG()
     end
})

FarmSetting:AddToggle('RemovePARTICLES', {
    Text = 'Remove Particles',
    Default = false, 
    Tooltip = 'Remove Particles',
    Callback = function()
        RemovePARTICLES()
     end
})

FarmSetting:AddSlider('KASlider', {
    Text = 'Kill Aura',
    Default = 1.25,
    Min = 0.25,
    Max = 5,
...
local Farm = TabboxFarms:AddTab("[ Farms ]")

Farm:AddLabel('Auto Farms')

Farm:AddToggle('FarmBosses', {
    Text = 'Farm Bosses [Map 2]',
    Default = false, 
    Tooltip = 'Kill All Bosses In Map 2',
    Callback = function(value)
        getgenv().AllBosses = value
    end
})

Farm:AddToggle('AutoCollectLily', {
...
Farm:AddToggle('Ignore', {
    Text = 'Disable Mouse',
    Default = false, 
    Tooltip = 'Disable Mouse [Use this before using Kill Aura]',
    Callback = function(value)
        if value then
            DisableMouseClicks()
        else
...
Farm:AddToggle('KillAura', {
    Text = 'Kill Aura [14 Hit] ',
    Default = false, 
    Tooltip = 'Safe Kill Aura', 
    Callback = function(value)
...
Farm:AddToggle('KillAura', {
    Text = 'Kill Aura [16 Hit] ',
    Default = false, 
    Tooltip = 'IDK Kill Aura', 
    Callback = function(value)
...
Farm:AddToggle('ThunderKa', {
    Text = 'Thunder Kill Aura',
    Default = false,
    Tooltip = 'Thunder Kill Aura',
    Callback = function(value)
        getgenv().TKA = value
    end
})

Farm:AddToggle('AutoBlock', {
...
Farm:AddToggle('AutoEatSouls', {
    Text = 'Auto Eat Souls [Demon]',
    Default = false, 
    Tooltip = 'Auto Eat Souls [Demon]',
    Callback = function(value)
        getgenv().AutoEatSouls = value
     end
})

local AutoChest = Tabs.Main:AddLeftGroupbox('[ Auto Chest ]')

AutoChest:AddToggle('AutoCollectChestv1', {
    Text = 'Auto Collect Chest v1',
    Default = false,
    Tooltip = 'Auto Collect Chest (Will Not Remove Chest After Collecting)',
    Callback = function(value)
        getgenv().AutoCollectChest = value
    end
})

AutoChest:AddToggle('AutoCollectChestv2', {
    Text = 'Auto Collect Chest v2',
    Default = false, 
    Tooltip = 'Auto Collect Chest (Will Remove Chest After Collecting)', 
    Callback = function(value)
        getgenv().AutoCollectChestv2 = value
    end
})

AutoChest:AddInput('MyTextbox', {
    Default = 'Webhook URL',
    Numeric = false,
    Finished = false,
...
AutoChest:AddToggle('AutoCollectChestv3', {
    Text = 'Auto Collect Chest v3',
    Default = false,
    Tooltip = 'Auto Collect Chest [Webhook (BETA)] | Pls Use This With WebhookURL ',
    Callback = function(value)
...
local clanvsrace = Tabs.Main:AddRightGroupbox('[ Status (Clan and Race) ]')

local clanLabel = clanvsrace:AddLabel("Clan: N/A")

clanvsrace:AddDropdown('SelectClan', {
...
local raceLabel = clanvsrace:AddLabel("Race: N/A")

clanvsrace:AddDropdown('SelectRace', {
...
local BreathingLabel = clanvsrace:AddLabel("Breathing: N/A")
local ArtLabel = clanvsrace:AddLabel("Art: N/A")
...
local demonProgressLabel = clanvsrace:AddLabel("Demon Progress: N/A")
local breathingProgressLabel = clanvsrace:AddLabel("Breathing Progress: N/A")
...
local timeLabel = clanvsrace:AddLabel("Elapsed Time: 0s")
...
clanvsrace:AddButton("SPIN DEMON ART", function()
    local args = {
...
local TPTrainer = Tabs.Teleports:AddLeftGroupbox('[ Teleport Traniner ]')

TPTrainer:AddToggle('TPtoTrainer', {
    Text = 'TP to Trainer',
    Default = false, -- Default value (true / false)
...
TPTrainer:AddDropdown('TPToTrainerr', {
...
TPTrainer:AddToggle('TPtoLocation', {
...
TPTrainer:AddDropdown('TPtoLocation', {
...
TPTrainer:AddButton("Teleport to Muzan", TPtoMuzan)

TPTrainer:AddButton("Reset To Void", function()
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(755, -528, 892)
end)

local isAutoBuyEnabled = false
local autoBuyLoop

local ServerTp = Tabs.Teleports:AddRightGroupbox('[ Server ]')

ServerTp:AddButton("Server Hop", function()
    Hop()
end)

ServerTp:AddButton("Join Lowest Server", function()
    local PlaceID = game.PlaceId
 local AllIDs = {}
 local foundAnything = ""
 local actualHour = os.date("!*t").hour
...
ServerTp:AddButton("Rejoin", function()
    game:GetService("TeleportService"):Teleport(game.PlaceId, game:GetService("Players").LocalPlayer)
end)

ServerTp:AddButton("Teleport To Hub", function()
    game:GetService('TeleportService'):Teleport(9321822839)
end)

ServerTp:AddButton("Teleport To Map 1", function()
    game:GetService('TeleportService'):Teleport(6152116144)
end)

ServerTp:AddButton("Teleport To Map 2", function()
    game:GetService('TeleportService'):Teleport(11468159863)
end)
 
ServerTp:AddButton("Teleport To Trading", function()
    game:GetService('TeleportService'):Teleport(13489082242)
end)

local Test = Tabs.Misc:AddLeftGroupbox('[ Miss ]')
...
Test3:AddToggle('UniGodMode', {
 ...
 Test3:AddToggle('Furiosity Mode',{
    Text = 'Furiosity Mode [All Race]',
    Default = false,
    Tooltip = 'Furiosity Mode',
    Callback = function(state)
        if state then
            game:GetService("ReplicatedStorage").Remotes.furiosity_activation123asd:FireServer(true)
        else
            game:GetService("ReplicatedStorage").Remotes.furiosity_activation123asd:FireServer(false)
        end
    end 
 })
...
local Demon = Tabs.Misc:AddRightGroupbox('[ God Modes (Demon) ]')

Demon:AddToggle('ShockGM', {
...
local HumanvsSlayer = Tabs.Misc:AddRightGroupbox('[ God Modes (SLAYER/HUMAN) ]')

HumanvsSlayer:AddToggle('SoundGM', {
...
HumanvsSlayer:AddToggle('FlameGM', {
...
HumanvsSlayer:AddToggle('BeastGM', {
...
HumanvsSlayer:AddToggle('InsectGM', {
...
HumanvsSlayer:AddToggle('WindGM', {
...
HumanvsSlayer:AddToggle('WaterGM', {
